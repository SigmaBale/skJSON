...
//     cr_assert(skJson_type(&current->value) == SK_MEMBER_NODE);
...
//     cr_assert_str_eq(temp->value.data.j_string,
//     "p2QAMXNIC1TJYWeIOttrVc98/R1BUFWu3/LiyKgUfQM"); cr_assert_str_eq(
...
//     cr_assert_str_eq(temp->value.data.j_string,
//     "+DiY3wvvV6TuJJhbpZisF/zLDA0zPMSvHdkr4UvCOqU"); cr_assert_str_eq(
//         (tempkey = skJson_string_value(&temp->value)),
...
//     cr_assert(skJson_type(skJson_objtuple_value(temp)) == SK_STRING_NODE);
//     cr_assert_str_eq(temp->value.data.j_string,
//     "uNiVztksCsDhcc0u9e8BujQXVUpKZIDTMczCvj3tD2s"); cr_assert_str_eq(
//         (tempkey = skJson_string_value(&temp->value)),
