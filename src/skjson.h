...
/* Json Element types */
#define SKJS_ERR    (0)         /* Error Json */
#define SKJS_OBJ    (1 << 1)    /* Object Json */
#define SKJS_ARR    (1 << 2)    /* Array Json */
#define SKJS_STR    (1 << 3)    /* String Json */
#define SKJS_REF    (1 << 4)    /* Reference Json */
#define SKJS_INT    (1 << 5)    /* Integer Json */
#define SKJS_DBL    (1 << 6)    /* Double Json */
#define SKJS_BOOL   (1 << 7)    /* Boolean Json */
#define SKJS_NULL   (1 << 8)    /* Null Json */
#define SKJS_NONE   (1 << 9)    /* None Json (empty) */
...
/* Return value of 'json' skJsonBool element */
...
/* Transforms the 'json' element into Json skJsonBool element with value 'boolean' */
 ...
 * Returns NULL on failure or pointer to the serialized json on success.
 * On failure serialization buffer is destroyed. */
PUBLIC(unsigned char*) skJson_serialize_with_bufsize(skJson* json, size_t n, skJsonBool expand);
...
PUBLIC(unsigned char*) skJson_serialize_with_buffer(skJson* json, unsigned char* buffer, size_t size, skJsonBool expand);
...
/* Sets the new skJsonBool value 'boolean' for 'json' element, returns true
 * if the 'json' value was changed, false if 'json is not skJsonBool element. */
...
/* Duplicates 'string' and sets the new value for 'json' element freeing the old string
...
PUBLIC(skJsonBool) skJson_object_insert_element(skJson *json, const char *key, skJson *elementp, size_t index);
...
PUBLIC(skJsonBool) skJson_object_insert_ref(skJson* json, const char *key, const char *ref,  size_t index);
PUBLIC(skJsonBool) skJson_object_insert_string(skJson* json, const char *key, const char *string,  size_t index);
/* Push key-value pairs at the end of the 'json' object. */
PUBLIC(skJsonBool) skJson_object_push_element(skJson *json, const char *key, skJson *elementp);
...
PUBLIC(skJsonBool) skJson_object_push_string(skJson* json, const char *key, const char *string);
...
PUBLIC(skJsonBool) skJson_object_sort_by(skJson* json, CmpFn cmp_fn);
...
PUBLIC(skJsonBool) skJson_object_is_sorted_by(skJson* json, CmpFn cmp_fn);
...
PUBLIC(skObjTuple*) skJson_object_index_by_key(const skJson* json, const char* key, skJsonBool sorted);
...
PUBLIC(skJsonBool) skJson_object_contains(const skJson* json, const char* key, skJsonBool sorted);
/* Get element associated with the 'key' from the 'json' object.
 ...
 * Return NULL if element was not found or input arguments are invalid */
PUBLIC(skJsonBool) skJson_object_remove_by_key(skJson* json, const char* key, skJsonBool sorted);
